...
const LIVE_WORLD_SYNC_MS = 10_000;
const LIVE_WORLD_RING = 6;
...
function effectiveOwner(cell: WorldCell): { id: string | null; name: string | null } {
  return {
    id: cell.building?.ownerId ?? cell.claim?.ownerId ?? null,
    name: cell.building?.ownerName ?? cell.claim?.ownerName ?? null,
  };
}

function liveTerritoryData(
  cells: WorldCell[],
  playerId: string,
    ...
    features: cells.flatMap((cell) => {
      ...
      try {
        ...
        return [{
          ...
          id: `live-territory-${cell.h3Index}`,
          properties: {
...
export function IndustrialMapLayer({
  ...
  const [liveCells, setLiveCells] = useState<WorldCell[]>(cells);
  const cellsRef = useRef<WorldCell[]>(cells);
  ...
  useEffect(() => {
    ...
    const syncWorld = async () => {
      if (cancelled || appState !== 'active' || syncInFlightRef.current) return;

      const origin = cellsRef.current.find((cell) => cell.distance === 0) ?? cellsRef.current[0];
      if (!origin) return;

      syncInFlightRef.current = true;
      try {
        const response = await locateWorld(origin.center.lat, origin.center.lng, LIVE_WORLD_RING);
        if (cancelled) return;
        cellsRef.current = response.cells;
        setLiveCells(response.cells);
      } catch {
        // Keep the last shared-world snapshot. The primary map already surfaces
        // connection errors, so background multiplayer sync stays unobtrusive.
      } finally {
        syncInFlightRef.current = false;
      }
    };

    const timer = setInterval(() => void syncWorld(), LIVE_WORLD_SYNC_MS);
    const stateSubscription = AppState.addEventListener('change', (nextState) => {
      const becameActive = appState !== 'active' && nextState === 'active';
      appState = nextState;
      if (becameActive) void syncWorld();
    });
  ...
  const hasConstruction = liveCells.some((cell) => isUnderConstruction(cell, now));
  ...
  const territoryData = useMemo(
    () => liveTerritoryData(liveCells, playerId, selectedH3, showOwned, showRivals),
    [liveCells, playerId, selectedH3, showOwned, showRivals],
  );

  const data = useMemo(
    () => visible
      ? industrialData(liveCells, playerId, selectedH3, now, showOwned, showRivals)
      : ({ type: 'FeatureCollection', features: [] } as FeatureCollection<Point>),
    [liveCells, now, playerId, selectedH3, showOwned, showRivals, visible],
  );

  if (!visible && !showOwned && !showRivals) return null;

  return (
    <>
      <GeoJSONSource
        id="live-multiplayer-territories"
        data={territoryData}
        onPress={(event) => {
      ...
      >
        <Layer
          id="live-multiplayer-territory-fill"
          type="fill"
          paint={{
            'fill-color': [
              'case',
        ...
        />
        <Layer
          id="live-multiplayer-territory-outline"
          type="line"
          paint={{
            'line-color': [
              'case',
